...
from rna_property.main import get_rna_property


with open(Path(__file__).parent.parent.parent / 'config.json') as handle:
...
def read_manifest(file_type: str) -> Dict[str, str]:
    read = False
    dic = dict()
    with open(Path(__file__).parent.parent / 'manifest.txt') as handle:
        for line in handle:
            line = line.strip()
...
def annotate(file: str, to_dir: str):
    ...
    rna_p = get_rna_property(file, to_dir)
    for c in rna_p.columns:
        summary[c] = rna_p[c].values
